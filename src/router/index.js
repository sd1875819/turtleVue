    ...
    children: [  /*嵌套路由就是访问上一层里的path+children里的path就可以访问children里component指定的页面嵌套到上一层的component页面里的总页面了，即包含了头部、侧边栏即主体的页面*/
      {
        ...
        该处我们是需要将User页面嵌套到上一层路由/home指定的Layout框架页中：通过http://localhost:8081/home/homeView访问页面
        但是该处有个疑问，该处写成path: '/user'后，通过访问http://localhost:8081/homeView也是可以将homeView嵌套到Layout框架页中正常显示*/
        path: 'user',
        name: 'User',
        component: () => import("@/views/User"),  /*，该处是导入页面的另外一种方式，将User界面进行二次路由导入*/
      },
      {
        path: 'book',
        name: 'Book',
